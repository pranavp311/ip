...
(cd "$TEST_DIR" && printf 'list\nbye\n' | java -cp "$TEST_DIR/classes" Kopi > "$TEST_DIR/reloaded-output.txt")

grep -Fq '1. [D][ ] return book (by: Sunday)' "$TEST_DIR/reloaded-output.txt"
grep -Fq '2. [E][ ] meeting (from: Mon to: Tue)' "$TEST_DIR/reloaded-output.txt"
